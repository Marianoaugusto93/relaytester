...
// ─── CSS ────────────────────────────────────────────────────────────────────
const WF_CSS = `
.wfd-wrap{display:flex;flex-direction:column;gap:6px;padding:8px;height:100%;min-height:0;overflow:hidden}
.wfd-canvas-box{position:relative;border-radius:8px;overflow:hidden;background:#0a0e14;border:1px solid rgba(255,255,255,.07);flex-shrink:0}
...
.wfd-timeline{border-radius:6px;overflow:hidden;background:#0a0e14;border:1px solid rgba(255,255,255,.07);flex-shrink:0}
 ...
 */
function drawWaveform(ctx, w, h, phasors, tNow, running, tripTimeS, tripLabel, freq) {
  ...
  const quarterPeriodPx = plotW / (WINDOW_MS / 1000 * freq * 4);
  for (let i = 0; i <= WINDOW_MS / 1000 * freq * 4; i++) {
  ...
  const tStart = tNow - WINDOW_MS / 1000;
  ...
  phases.forEach(ph => {
    ...
    for (let px = 0; px <= plotW; px++) {
      const t = tStart + (px / plotW) * (WINDOW_MS / 1000);
...
export default function WaveformDisplay({ phasors, isInjecting, injectionTime, tripHistory, freq = 60 }) {
  ...
  const animate = useCallback(() => {
    ...
    if (startWallRef.current !== null) {
      tNowRef.current = (now - startWallRef.current) / 1000;
    }
    ...
    drawWaveform(wfCtx, wfW, wfH, ph, tNowRef.current, runningRef.current, tripInfo.timeS, tripInfo.label, freq);
  ...
  useEffect(() => {
    ...
    const ph = phasors?.currents || { Ia: { mag: 0, ang: 0 }, Ib: { mag: 0, ang: -120 }, Ic: { mag: 0, ang: 120 } };
    const tripInfo = tripInfoRef.current;
    drawWaveform(wfCtx, wfW, wfH, ph, tNowRef.current, false, tripInfo.timeS, tripInfo.label, freq);
  ...
  return (
    <>
      ...
      <div className="wfd-wrap">
        ...
        {/* Waveform canvas */}
        <div className="wfd-canvas-box" style={{ height: CANVAS_H }}>
          <canvas ref={wfCanvasRef} className="wfd-canvas" style={{ width: '100%', height: CANVAS_H }}/>
        ...
        {/* Timeline */}
        <div className="wfd-timeline" style={{ height: TIMELINE_H }}>
          <canvas ref={tlCanvasRef} className="wfd-timeline" style={{ width: '100%', height: TIMELINE_H }}/>
        </div>
