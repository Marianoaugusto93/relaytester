...
const FL_APP_VERSION='v6.4.9 R13';
const FL_FILE_TAG='v6_4_9_R13';
const FL_CURVE_ENGINE_VERSION='v6.4.9 R13 Engine Curvas Pro';
...
function renderCurveEngineBox64(d){
  ...
  if(box) box.innerHTML=`<div class="coordSideTitle">Motor de Curvas v6.4.9 R13</div><div class="coordStats"><div class="coordStat"><b>IDMT</b><span>${idmt}</span></div><div class="coordStat"><b>TD/50</b><span>${td}</span></div><div class="coordStat"><b>Pontos</b><span>${pts}</span></div><div class="coordStat"><b>Início IDMT</b><span>${String(model.startFactor).replace('.',',')} × pickup</span></div><div class="coordStat"><b>Neutro</b><span>${model.neutralMode}</span></div><div class="coordStat"><b>Alertas</b><span>${alerts} / Erros ${errs}</span></div></div>`;
  if(audit){
...
function runInternalJsonTestsV630(){
  ...
  return {version:'v6.4.9 R13',generatedAt:new Date().toISOString(),total:results.length,passed:results.filter(r=>r.pass).length,failed:results.filter(r=>!r.pass).length,results};
}
...
function renderDiagnosticoAvancado54(d){
  ...
  return '<div class="divider">Diagnóstico avançado de seletividade v6.4.9 R13</div><div class="diagBox">'+itens.map(x=>`<div class="diagCard ${x.nivel}"><b>${icon[x.nivel]||'Info'} — ${x.titulo}</b><br>${x.achado}<br><span class="small">${x.acao}</span></div>`).join('')+'</div>';
}
...
function exportarPNGCoordenogramaClassicoHD(d=calcData(false)){
  ...
  a.download='coordenograma_relaytester_protecao_sep_tela_HD.png';
...
function buildCoordRenderModel(d=calcData(false)){
  ...
  return {
    ...
    title:str('coordTitle')||`Relaytester Protecao SEP ${FL_APP_VERSION} - Coordenograma`,
...
function renderAuditBox(d){
  ...
  const revisaoRows=[
    row(str('revisao')||'Rev. 00',str('dataEstudo')||new Date().toISOString().slice(0,10),str('responsavel')||'Respons&aacute;vel n&atilde;o informado','Emiss&atilde;o preliminar gerada pelo Relaytester v6.4.9 R13')
  ].join('');
  ...
  $('report').innerHTML=`<div class="reportA4">
    <div class="cover">${logo}<div class="docType">Relat&oacute;rio t&eacute;cnico preliminar</div><h1>Estudo de Prote&ccedil;&atilde;o e Seletividade</h1><p><strong>${str('cliente')||'Cliente/obra n&atilde;o informado'}</strong></p>
      <div class="metaGrid"><div class="meta"><b>Empresa elaboradora</b>${str('empresa')}</div><div class="meta"><b>Respons&aacute;vel t&eacute;cnico</b>${str('responsavel')} - ${str('registro')}</div><div class="meta"><b>Concession&aacute;ria</b>${str('concessionaria')}</div><div class="meta"><b>Rel&eacute; / equipamento</b>${str('rele')}</div><div class="meta"><b>Data</b>${str('dataEstudo')}</div><div class="meta"><b>Revis&atilde;o</b>${str('revisao')}</div><div class="meta"><b>Documento base</b>${str('norma')}</div><div class="meta"><b>Software</b>Relaytester Prote&ccedil;&atilde;o SEP v6.4.9 R13</div></div>
    </div>
    ...
    <div class="auditSummary ${audit.classe}"><strong>Matriz de emiss&atilde;o v6.4.9 R13:</strong> ${audit.nivel}. ${audit.recomendacao}</div>
...
function abrirRelatorioVetorial(){
  ...
  *{box-sizing:border-box}
  ...
  w.document.open();
  w.document.write(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>Relaytester Proteção SEP v6.4.9 R13 — Motor Mesh de Curvas Compostas</title><style>${css}</style></head><body>${report.outerHTML}</body></html>`);
  w.document.close();
